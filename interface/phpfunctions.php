...
function savelead($data)
{
	...
	if ($isdone == 0)
        ...
        if ($data['notcalled'] != 1){  // skip if not called
	...
	while ($row = mysql_fetch_assoc($dsendres))
		...
		$mailer = new mailer($mailtype,$params);	
		$mailer->mailfrom = $row['emailfrom'];
		$attachments = split(",",$row['attachments']);
		$mailer->mailto =$data['email'];
		$mailer->subject = $row['template_subject'];
		$mailer->message=$row['template_body'];

 		foreach ($attachments as $attachment)
		{
                    $mailer->attach("../attachments/".$attachment);
		}
                $a = $mailer->send();
		
		if ($a)
			{
				mysql_query("INSERT into email_log set userid = '$auid', mailto = '$email', message = '".$row['template_body']."', timesent = NOW(), projectid = '".$row['projectid']."', leadid = '".$data['lid'].", status = '$a'");
			}
                ...
                if (mysql_num_rows($cres) < 1)
                {
                  if ($tocat == 'agent' || $tocat == 'team')
                    {  
                     ...
                     else  mysql_query("INSERT into statuses set projectid = '".$nlist['projects']."', statusname = '".$data['disposition']."', statustype = 'text', category = 'callable' ");
                }
